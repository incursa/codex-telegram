...
- Host updates, when enabled, are private-chat-only and require an explicit confirmation after the local worker is drained and idle. The bot writes a bounded, atomically replaced request for an operator-owned updater; it never executes package-manager commands, service-manager commands, shell text, package URLs, or installation paths received from Telegram. The Debian updater uses only the operator-configured APT sources, caches the previous signed package, verifies the restarted service through its local health endpoint, and attempts automatic rollback on failure. Keep the request/state/cache directories protected so only the bot and root-owned updater service can read or write them, and retain SSH or local service-manager access as an out-of-band recovery path.
