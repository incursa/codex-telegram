...
internal sealed class TelegramCodexBotCommandHandler : ITelegramCodexBotUpdateHandler
{
    ...
    {
        ResolvedSession resolved = await ResolveSessionAsync(message.ConversationScope, arguments, cancellationToken, message.UserId).ConfigureAwait(false);
        if (resolved.Session is null)
        {
            await ReplyAsync(sender, message, resolved.Message, null, cancellationToken).ConfigureAwait(false);
            return;
        ...
        bool planMode = false)
    {
        ...
        string traceId = message.TraceId ?? _traceStore.CreateTraceId();
        message = message with { TraceId = traceId };
        CodexSupervisionCommandStart supervision = await _supervisionLedger.StartCommandAsync(
            message.CommandId,
            session.Id,
            session.Name,
            message.ConversationScope,
            message.UserId,
            planMode ? "plan" : "prompt",
            cancellationToken).ConfigureAwait(false);
        if (!supervision.Accepted && supervision.Run is not null)
        {
            _logger.LogDebug(
                "Ignoring duplicate supervision command {CommandId} for task {TaskId} and run {RunId}.",
                supervision.CommandId,
                supervision.Run.TaskId,
                supervision.Run.RunId);
            return true;
        }

        message = message with { CommandId = supervision.CommandId };

        string? supervisionRunId = supervision.Run?.RunId;
        ...
        if (_turnCoordinator.HasActiveTurnForThread(session.Id))
        {
            _logger.LogDebug(
                ...
                message.MessageThreadId,
                session.Id);
            await SetSupervisionStateAsync(supervisionRunId, CodexSupervisionRunState.Queued, session.Id, null, "active_turn", cancellationToken).ConfigureAwait(false);
            await QueuePromptAsync(message, session, trimmed, sender, cancellationToken, planMode, supervisionRunId).ConfigureAwait(false);
            return true;
        }
        ...
        if (IsLive(session.Status))
        {
            _logger.LogDebug(
                ...
                session.Id,
                session.Status);
            await SetSupervisionStateAsync(supervisionRunId, CodexSupervisionRunState.Queued, session.Id, null, "session_active", cancellationToken).ConfigureAwait(false);
            await QueuePromptAsync(message, session, trimmed, sender, cancellationToken, planMode, supervisionRunId).ConfigureAwait(false);
            return true;
        }
        ...
        if (HasPendingOutboundForConversation(outboundStatus, message.ConversationScope))
        {
            _logger.LogDebug(
                ...
                message.ChatId,
                message.MessageThreadId);
            await SetSupervisionStateAsync(supervisionRunId, CodexSupervisionRunState.Queued, session.Id, null, "telegram_output_pending", cancellationToken).ConfigureAwait(false);
            await QueuePromptAsync(message, session, trimmed, sender, cancellationToken, planMode, supervisionRunId).ConfigureAwait(false);
            return true;
        }
        ...
        try
        {
            ...
            {
                _ = ObserveSlowTelegramSendAsync(sendTask, message, session, trimmed, sender, typingRegistration, planMode, supervisionRunId);
            ...
            await SetSupervisionStateAsync(supervisionRunId, CodexSupervisionRunState.Running, execution.ThreadId, execution.TurnId, null, cancellationToken).ConfigureAwait(false);
        ...
        bool planMode)
    {
        ...
        // The replacement is a deliberate child dispatch. Keep it distinct from the
        // original command so an uncertain external outcome is never silently replayed
        // under the same command identity.
        string recoveryCommandId = string.IsNullOrWhiteSpace(message.CommandId)
            ? $"command:recovery:{Guid.NewGuid():N}"
            : $"{message.CommandId}:recovery:{Guid.NewGuid():N}";
        return await SendOrQueueAsync(
            message with { CommandId = recoveryCommandId },
            replacement,
            text,
            sender,
            cancellationToken,
    ...
    private async Task<CodexThreadExecutionVm> StartSessionSendAsync(
        TelegramInboundMessage message,
        CodexSessionSummary session,
        string text,
        bool planMode,
        CancellationToken cancellationToken)
    {
        CodexSupervisionTaskSnapshot? task = await _supervisionLedger.GetTaskForSessionAsync(
            ...
            session.Id,
            cancellationToken).ConfigureAwait(false);
        if (task?.WorkerId is not null && _workerRegistry is not null)
        {
            ...
            if (!string.Equals(task.WorkerId, localWorker.WorkerId, StringComparison.Ordinal))
            {
                ...
                return await _remoteSessionRelay.SendAsync(
                    task,
                    message.UserId,
                    message.ChatId,
                    message.MessageThreadId,
        ...
        return message.Attachments is { Count: > 0 }
            ? await _sessionManager.SendAsync(
                ...
                cancellationToken).ConfigureAwait(false)
            : planMode
                ? await _sessionManager.SendPlanAsync(session.Id, text, cancellationToken).ConfigureAwait(false)
                : await _sessionManager.SendAsync(session.Id, text, cancellationToken).ConfigureAwait(false);
    }
    ...
    private async Task ObserveSlowTelegramSendAsync(
        ...
        bool planMode,
        string? supervisionRunId)
    {
        try
        {
            CodexThreadExecutionVm execution = await sendTask.ConfigureAwait(false);
            await SetSupervisionStateAsync(supervisionRunId, CodexSupervisionRunState.Running, execution.ThreadId, execution.TurnId, null, CancellationToken.None).ConfigureAwait(false);
            _followRegistry.FollowThread(message.ConversationScope, execution.ThreadId);
        ...
        catch (InvalidOperationException exception) when (exception.Message.Contains("already active", StringComparison.OrdinalIgnoreCase))
        {
            _logger.LogDebug(
                ...
                message.MessageThreadId,
                session.Id);
            await SetSupervisionStateAsync(supervisionRunId, CodexSupervisionRunState.Queued, session.Id, null, "active_turn", CancellationToken.None).ConfigureAwait(false);
            await QueuePromptAsync(message, session, text, sender, CancellationToken.None, planMode, supervisionRunId).ConfigureAwait(false);
        }
        catch (Exception exception)
        {
            await SetSupervisionStateAsync(supervisionRunId, CodexSupervisionRunState.Unknown, session.Id, null, "external_outcome_unknown", CancellationToken.None).ConfigureAwait(false);
    ...
    private async Task HandleSteerAsync(TelegramInboundMessage message, string arguments, ITelegramBotMessageSender sender, CancellationToken cancellationToken)
    {
        ...
        {
            if (_remoteSessionControlRelay is null)
            {
                await ReplyAsync(sender, message, "Remote session controls are not configured on this host.", null, cancellationToken).ConfigureAwait(false);
                return;
            }

            try
            {
                CodexRemoteSessionControlResponse result = await _remoteSessionControlRelay.SendAsync(
                    remoteTask,
                    message.UserId,
                    message.ChatId,
                    message.MessageThreadId,
                    message.CommandId ?? $"command:remote-steer:{Guid.NewGuid():N}",
                    "steer",
                    BuildCodexInputText(message, arguments),
                    cancellationToken).ConfigureAwait(false);
                await ReplyAsync(sender, message, $"Steering {resolved.Session.Name} {result.Message}.", null, cancellationToken).ConfigureAwait(false);
            }
            catch (InvalidOperationException exception)
            {
                await ReplyAsync(sender, message, exception.Message, null, cancellationToken).ConfigureAwait(false);
        ...
        if (remoteTask is not null)
        {
            ...
            try
            {
                CodexRemoteSessionControlResponse result = await _remoteSessionControlRelay.SendAsync(
                    ...
                    message.ChatId,
                    message.MessageThreadId,
                    message.CommandId ?? $"command:remote-stop:{Guid.NewGuid():N}",
                    "stop",
                    null,
                    cancellationToken).ConfigureAwait(false);
                await ReplyAsync(sender, message, $"{result.Message} for {resolved.Session.Name}. Pending queued messages for that session were cleared.", null, cancellationToken).ConfigureAwait(false);
            }
            catch (InvalidOperationException exception)
            {
                await ReplyAsync(sender, message, exception.Message, null, cancellationToken).ConfigureAwait(false);
        ...
        if (remoteTask is not null)
        {
            ...
            try
            {
                CodexRemoteSessionControlResponse result = await _remoteSessionControlRelay.SendAsync(
                    ...
                    message.ChatId,
                    message.MessageThreadId,
                    message.CommandId ?? $"command:remote-kill:{Guid.NewGuid():N}",
                    "kill",
                    null,
                    cancellationToken).ConfigureAwait(false);
                await ReplyAsync(sender, message, $"{result.Message} for {resolved.Session.Name}. Pending queued messages for that session were cleared.", null, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<ResolvedSession> ResolveActiveSessionAsync(
        TelegramConversationScope conversation,
        CancellationToken cancellationToken,
        long? ownerUserId = null)
    {
        ...
        if (session is null)
        {
            session = await GetRemoteSessionAsync(ownerUserId ?? conversation.ChatId, activeSessionId, cancellationToken).ConfigureAwait(false);
            if (session is null)
            {
                await _stateStore.ClearActiveSessionAsync(conversation, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<ResolvedSession> ResolveSessionAsync(
        TelegramConversationScope conversation,
        string arguments,
        CancellationToken cancellationToken,
        long? ownerUserId = null)
    {
        ...
        {
            return await ResolveActiveSessionAsync(conversation, cancellationToken, ownerUserId).ConfigureAwait(false);
        }
        ...
        foreach (CodexSupervisionTaskSnapshot task in await _supervisionLedger.ListTasksAsync(ownerUserId ?? conversation.ChatId, cancellationToken).ConfigureAwait(false))
        {
            if ((task.CodexThreadId.Equals(sessionId, StringComparison.OrdinalIgnoreCase)
                    || task.CodexThreadId.StartsWith(sessionId, StringComparison.OrdinalIgnoreCase))
                && matches.All(session => !session.Id.Equals(task.CodexThreadId, StringComparison.Ordinal)))
            {
                matches.Add(ToRemoteSession(task));
            }
        }
    ...
    private bool IsSessionInConfiguredRepository(CodexSessionSummary session)
        => session.IsRemote || !IsRepositoryMode || PathsEqual(session.WorkingDirectory, _configuredRepositoryPath);

    private async Task<CodexSessionSummary?> GetRemoteSessionAsync(
        long userId,
        string sessionId,
        CancellationToken cancellationToken)
    {
        CodexSupervisionTaskSnapshot? task = (await _supervisionLedger.ListTasksAsync(userId, cancellationToken).ConfigureAwait(false))
            .FirstOrDefault(candidate => string.Equals(candidate.CodexThreadId, sessionId, StringComparison.Ordinal));
        return task is null ? null : ToRemoteSession(task);
    }

    private async Task<CodexSupervisionTaskSnapshot?> GetRemoteTaskForSessionAsync(
        long userId,
        string sessionId,
        CancellationToken cancellationToken)
    {
