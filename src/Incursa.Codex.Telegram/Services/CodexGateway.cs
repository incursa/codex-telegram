...
internal interface ICodexGateway
{
    ...
    Task<CodexThreadGoalVm> SetThreadGoalAsync(string threadId, string objective, long? tokenBudget = null, CancellationToken cancellationToken = default);

    Task<CodexThreadGoalVm> SetThreadGoalStatusAsync(string threadId, CodexThreadGoalStatus status, CancellationToken cancellationToken = default);
