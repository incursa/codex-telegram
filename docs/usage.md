...
For isolated parallel work, use /task new [name] [| baseRef]. It provisions a task-owned Git worktree, branch, development port, and database namespace, then starts a new Codex session in that worktree. With a configured coordinator and explicitly admitted worker, `/task remote <workerId> [name] [| baseRef] [| recipeId]` provisions the same resources on that worker after authenticated lease and repository checks. The selected remote task accepts `/send <text>` and Plan mode through the authenticated coordinator/worker relay; the worker remains the Codex execution authority and duplicate command IDs do not start a second turn. Inspect it with /task status; stop the session before release, clean release requires confirmation, and dirty-worktree discard is a separate explicit command. Attachments and remote session controls are not yet relayed.
