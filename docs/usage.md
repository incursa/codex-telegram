...
When enabled by the operator, open the bot's Telegram menu button to view a mobile-friendly, Incursa UI Kit supervision dashboard. It shows `Needs attention`, recent activity, runtime/session/project context, task detail with bounded timeline, Codex-reported changes, safe artifact metadata, and worker readiness. From a task's review packet, you can acknowledge the exact evidence or prepare the existing `/handoff` command. Handoff preparation also returns bounded review and artifact metadata from the same redacted packet, and explicitly reports when the Codex runtime could not provide evidence. From Workers, explicitly confirmed Drain/Resume changes task admission without interrupting current sessions. Prompts, approvals, steering, and session/project changes remain in Telegram. An acknowledgement applies only to that task/run/packet, so a newer run appears again. If the surface says `Preview`, it is explicit local preview mode (`?preview=1`) and is not connected to a Telegram identity or live Codex data. If it says `Stale`, the last confirmed live snapshot is being shown while the host is unavailable.
The Fleet rollouts card in the Telegram-authenticated Mini App accepts an exact target version, package SHA-256, required capabilities, and explicit worker selection. Paired browser sessions can inspect rollout state but cannot stage or activate updates. Planning and each Advance/Finalize/Rollback action require confirmation. Advance stages at most one worker and stops until the external installer restarts it and posts health completion; it does not copy packages, stop services, or bypass worker drain/lease gates. Finalize succeeds only when every selected worker reports active target evidence. A health failure leaves rollback as an explicit reverse-order operation.
...
For isolated parallel work, use /task new [name] [| baseRef]. It provisions a task-owned Git worktree, branch, development port, and database namespace, then starts a new Codex session in that worktree. With a configured coordinator and explicitly admitted worker, `/task remote <workerId> [name] [| baseRef] [| recipeId]` provisions the same resources on that worker after authenticated lease and repository checks. The selected remote task accepts `/send <text>`, normal prompts with bounded attachments, Plan mode, `/steer`, `/stop`, and confirmed `/kill` through the authenticated coordinator/worker relay; the worker remains the Codex execution authority and duplicate command IDs do not repeat a turn or control operation. Plan mode with attachments is rejected. Model/reasoning and goal controls relay to the worker through separate owner- and lease-bound settings operations. `/task status` and confirmed `/task release`/`discard` execute on the worker and return bounded workspace evidence without returning its filesystem path. The Mini App can inspect the worker-owned task detail, timeline, review packet, and artifact metadata through the same redacted relay.
...
If the optional host-update handoff is enabled, wait for active turns to finish, then use `/worker drain confirm`, `/update status`, and `/update confirm` instead of attempting to run package-manager commands through Telegram. The bot first reports that the external updater is starting, then reports completion only after the Debian updater restarts and health-checks the service. It remains disabled for portable/manual installs unless an equivalent operator-owned updater is installed.
