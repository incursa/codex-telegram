...
For isolated parallel work, use /task new [name] [| baseRef]. It provisions a task-owned Git worktree, branch, development port, and database namespace, then starts a new Codex session in that worktree. With a configured coordinator and explicitly admitted worker, `/task remote <workerId> [name] [| baseRef] [| recipeId]` provisions the same resources on that worker after authenticated lease and repository checks. The selected remote task accepts `/send <text>`, Plan mode, `/steer`, `/stop`, and confirmed `/kill` through the authenticated coordinator/worker relay; the worker remains the Codex execution authority and duplicate command IDs do not repeat a turn or control operation. `/task status` and confirmed `/task release`/`discard` execute on the worker and return bounded workspace evidence without returning its filesystem path. The Mini App can inspect the worker-owned task detail, timeline, review packet, and artifact metadata through the same redacted relay. Attachments and model/goal controls are not yet relayed.
