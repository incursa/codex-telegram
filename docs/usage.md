...
When enabled by the operator, open the bot's Telegram menu button to view a mobile-friendly, Incursa UI Kit supervision dashboard. It shows `Needs attention`, recent activity, runtime/session/project context, and task detail with bounded timeline, Codex-reported changes, and safe artifact metadata. From a task's review packet, you can acknowledge the exact evidence or prepare the existing `/handoff` command; prompts, approvals, steering, and session/project changes remain in Telegram. An acknowledgement applies only to that task/run/packet, so a newer run appears again. If the surface says `Preview`, it is explicit local preview mode (`?preview=1`) and is not connected to a Telegram identity or live Codex data. If it says `Stale`, the last confirmed live snapshot is being shown while the host is unavailable.
