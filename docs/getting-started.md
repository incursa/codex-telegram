...
The host includes a disabled-by-default Mini App companion surface. It opens directly on the session selected in that Telegram conversation and is a settings/live-status panel for that session: session identity, observed activity, goal, model/thinking settings, account quota usage, and shared global instructions. There is no prompt composer or alternate conversation workflow; prompts and approvals remain in Telegram. Review, worker, handoff, and fleet surfaces remain secondary capabilities documented in the staged [supervision workspace roadmap](mini-app-roadmap.md).
...
The current Mini App exposes bounded session settings actions. Renaming, goal changes, and model/thinking changes are authenticated to the conversation's current session; model/thinking changes during an active turn are labeled for the next turn. Global instructions are saved as a shared host profile and are applied at a new session or the next supported safe boundary—saving them never sends a prompt or interrupts work. Use the Telegram chat for creating sessions, changing projects, sending prompts, steering turns, approvals, retries, and cancellation. Failed refreshes show disconnected or stale state; sample data is only available with an explicit `?preview=1` query string. Do not expose the listener beyond the intended HTTPS proxy or tunnel.

To allow the same dashboard in a normal browser, set `BrowserPairingEnabled` to `true`. Open the Mini App URL in that browser, copy its short-lived pairing code, and send `/pair <code>` to the bot in the authorized private Telegram chat. The browser receives a time-limited session without weakening Telegram authorization; use `/pair status` to inspect sessions and `/pair revoke` to revoke all browser sessions for the Telegram user. Pairing is disabled by default.

Each host also has a local worker identity used by task-workspace scheduling. Configure `CodexTelegram:Worker:WorkerId` and `DisplayName` when you want stable operator-facing names; otherwise the host generates and persists an ID in `codex-worker-state.json`. `MaxConcurrentTasks` bounds leases on that worker. Use `/worker status` or the Mini App Workers card to inspect readiness and capacity. Explicitly confirm Drain before maintenance so new task claims stop while current work remains untouched; Resume re-enables claims. The Mini App applies the same local control or authenticated coordinator relay for admitted remote workers.
...
To register several isolated hosts with one self-hosted coordinator, enable `CodexTelegram:Coordinator:Enabled` on the coordinator and provide a private token. On each worker, enable `WorkerRegistrationEnabled`, point `Url` at the coordinator's base HTTPS URL, set `Worker:ControlPlaneUrl` to that worker's private endpoint, and provide the same token. Configure exact `AllowedWorkerIds` on the coordinator when you want explicit worker admission. The coordinator shows fresh worker readiness and lease capacity in the Mini App; its confirmed Drain/Resume controls relay only bounded worker state changes and do not receive Codex credentials or execute tasks on behalf of a worker. In Repository mode, configure the same admitted repository root on the worker, then use `/task remote <workerId> [name] [| baseRef] [| recipeId]` to provision a task there. The worker owns its local worktree, session, lease, and Codex execution. After provisioning, `/send <text>`, normal prompts with bounded attachments, Plan mode, `/steer`, `/stop`, and confirmed `/kill` can use the authenticated relay. Model/reasoning and goal controls use separate authenticated worker operations. Plan mode with attachments is not supported remotely. `/task status` and confirmed `/task release`/`discard` also execute on the worker and return bounded workspace evidence; remote review/file actions remain separate follow-up capabilities.
The Telegram-authenticated Mini App also provides fleet rollout planning when worker updates are enabled. Enter the exact target version, package SHA-256, required capabilities, and selected WorkerIds; then advance one drained, idle worker at a time. The external installer remains responsible for staging into the protected installation, restarting the service, and posting `/api/worker/v1/update/complete`. Paired browsers may inspect rollout state but cannot invoke rollout actions.
...
Task workspace provisioning is available through the R3 task flow. Set CodexTelegram:Workspace:TaskWorktreeRoot to a writable operator-owned directory when using managed worktrees, and keep TaskDevelopmentPortRangeStart/TaskDevelopmentPortRangeEnd inside an unused local range. Use `/task new [name] [| baseRef]` after selecting a project; it creates a new Codex session in the allocated worktree and records the task against the current authorized Telegram conversation. For a coordinator, use `/task remote <workerId> [name] [| baseRef] [| recipeId]` after configuring the worker in Repository mode with the admitted repository root; this creates the worktree and session on that worker, and subsequent text/Plan/steer/stop/kill operations are forwarded through the authenticated worker boundary. Stop the session before `/task release <taskId> confirm`; `/task discard <taskId> confirm` is the separate explicit path for discarding dirty work. Remote workspace lifecycle is not relayed yet. The flow does not silently move an existing session or replay a prompt.
