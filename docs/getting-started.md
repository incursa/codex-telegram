...
Task workspace provisioning is available through the R3 task flow. Set CodexTelegram:Workspace:TaskWorktreeRoot to a writable operator-owned directory when using managed worktrees, and keep TaskDevelopmentPortRangeStart/TaskDevelopmentPortRangeEnd inside an unused local range. Use `/task new [name] [| baseRef]` after selecting a project; it creates a new Codex session in the allocated worktree and records the task against the current authorized Telegram conversation. For a coordinator, use `/task remote <workerId> [name] [| baseRef] [| recipeId]` after configuring the worker in Repository mode with the admitted repository root; this creates the worktree and session on that worker. Stop the session before `/task release <taskId> confirm`; `/task discard <taskId> confirm` is the separate explicit path for discarding dirty work. The flow does not silently move an existing session or replay a prompt.
