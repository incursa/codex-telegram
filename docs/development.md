...
The Mini App is a static frontend under `src/Incursa.Codex.Telegram/wwwroot`. Its GET API projects existing Codex threads and turns; its bounded write actions acknowledge an exact review packet, prepare a Telegram handoff, or explicitly drain/resume a worker. It is not a competing task store or Codex execution authority. When changing its layout or Telegram WebApp integration, validate the actual served page at compact, full-height, and fullscreen viewport sizes. Check horizontal overflow, dynamic safe-area padding, the viewport badge, the fullscreen control, activation/reopen refresh behavior, task-detail navigation, stale-data handling, worker confirmation controls, and keyboard access with a real browser harness when available; a successful .NET publish does not establish frontend behavior. The staged product contract is in [mini-app-roadmap.md](mini-app-roadmap.md).
