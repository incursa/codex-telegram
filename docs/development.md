...
The Mini App is a static frontend under `src/Incursa.Codex.Telegram/wwwroot`. Its bootstrap is session-first: it projects the conversation's selected session, observed active-turn status, Codex-backed goal state, owning-worker model/thinking catalog, account quota windows/history, and global instruction application state. Authenticated writes are limited to renaming the current session, goal lifecycle, next-turn model/thinking settings, selecting a secondary session, and saving the shared global instruction profile; prompts and approvals remain in Telegram. Global instruction and quota history files live in the operator-owned data root. When changing its layout or Telegram WebApp integration, validate the actual served page at compact, full-height, and fullscreen viewport sizes. Check horizontal overflow, dynamic safe-area padding, the viewport badge, the fullscreen control, activation/reopen refresh behavior, the current-session and no-session states, status labels, settings acceptance text, global-instruction safe-boundary messaging, quota sparklines, and keyboard access with a real browser harness when available; a successful .NET publish does not establish frontend behavior. The staged product contract is in [mini-app-roadmap.md](mini-app-roadmap.md).
