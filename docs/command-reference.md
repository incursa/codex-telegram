...
Creates a task on one explicitly selected registered worker. The coordinator first obtains an idempotent lease handoff, then asks the worker to validate its identity, repository admission, recipe version, and local readiness before creating the worktree and Codex session. The worker returns only bounded ownership and allocated-resource metadata; repository paths and Codex execution remain local to that worker. After creation, `/send <text>`, normal prompts with bounded attachments, Plan mode, `/steer`, `/stop`, confirmed `/kill`, and task workspace status/release can target the selected worker through authenticated relays. Model/reasoning controls and goal lifecycle operations also relay to the owning worker. Plan mode with attachments is rejected because that combination is not supported by the remote execution contract. The worker records durable command identity before execution and forwards bounded turn events back to the coordinator.
...
Shows the local worker's readiness, capabilities, heartbeat, and task-lease capacity. `drain` stops new task claims while allowing existing work to finish; `resume` permits new claims again. Drain and resume require an authorized private chat and explicit confirmation. `/task remote` uses coordinator-issued lease handoff to provision a task on one explicitly selected worker. `/send`, normal prompts with bounded attachments, Plan mode, `/steer`, `/stop`, and confirmed `/kill` can then use the authenticated worker relay. `/task status`, `/task release <taskId> confirm`, and `/task discard <taskId> confirm` also relay to the owning worker; only bounded workspace evidence returns to the coordinator. Model/reasoning and goal controls use the authenticated settings relay. Plan mode with attachments remains unsupported remotely.
...
### `/update status` / `/update confirm` / `/update cancel confirm` / `/update rollback confirm`

Requests, inspects, or recovers an update of the primary Codex Telegram host. This command is disabled unless `CodexTelegram:HostUpdate:Enabled` is true. It is available only from an authorized private chat. Update and rollback requests require no active Codex turn plus a worker that has already been drained with zero active task leases; cancelling a queued request does not require another drain.

`/update confirm` writes a durable request for the root-owned Debian `codex-telegram-updater` service. `/update cancel confirm` removes only a queued `Requested`/`RollbackRequested` request while holding the shared update lock; it refuses to interfere with an updater that has already entered `Applying`. If the running version changed through a manual APT upgrade, `/update status` automatically reconciles the old request, removes it from the updater queue, and reports the manually detected result. `/update rollback confirm` writes an explicit rollback request. The Telegram process never invokes APT, `dpkg`, `systemctl`, a shell, or an arbitrary installer path. The updater owns package verification, caching the last-known-good `.deb`, the normal APT transaction, service restart, `/health` verification, and automatic rollback after a failed update. After a successful restart or rollback, the new process reports the terminal result to the conversation that requested the update. Ordinary `apt upgrade` remains an independent supported path and does not depend on Telegram.
