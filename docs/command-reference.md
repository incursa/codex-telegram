...
### `/update status` / `/update confirm` / `/update rollback confirm`

Requests or inspects an update of the primary Codex Telegram host. This command is disabled unless `CodexTelegram:HostUpdate:Enabled` is true. It is available only from an authorized private chat, and mutating operations require no active Codex turn plus a worker that has already been drained with zero active task leases.

`/update confirm` writes a durable request for the root-owned Debian `codex-telegram-updater` service. `/update rollback confirm` writes an explicit rollback request. The Telegram process never invokes APT, `dpkg`, `systemctl`, a shell, or an arbitrary installer path. The updater owns package verification, caching the last-known-good `.deb`, the normal APT transaction, service restart, `/health` verification, and automatic rollback after a failed update. After a successful restart or rollback, the new process reports the terminal result to the conversation that requested the update. Ordinary `apt upgrade` remains an independent supported path and does not depend on Telegram.
