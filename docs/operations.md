...
For a coordinator deployment, set `CodexTelegram:Coordinator:Enabled` and a private `AuthenticationToken` on the coordinator host. Set `WorkerRegistrationEnabled`, the coordinator `Url`, and the same token on each worker; optionally list exact `AllowedWorkerIds` on the coordinator. Workers send bounded health heartbeats to `/api/coordinator/v1/workers/heartbeat` over HTTP(S). The endpoint accepts only the exact bearer token and bounded worker snapshots; stale workers appear unavailable in the read-only Mini App. Keep the URL private or behind the existing operator-controlled HTTPS boundary, and rotate the token as an operator-managed secret. This control plane registers and observes workers; task execution remains on the worker and no Codex credentials are transferred. It does not yet dispatch a task or move a lease between workers.
