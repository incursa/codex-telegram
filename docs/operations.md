...
For a coordinator deployment, set `CodexTelegram:Coordinator:Enabled` and a private `AuthenticationToken` on the coordinator host. Set `WorkerRegistrationEnabled`, the coordinator `Url`, and the same token on each worker; optionally list exact `AllowedWorkerIds` on the coordinator. Workers send bounded health heartbeats to `/api/coordinator/v1/workers/heartbeat` over HTTP(S). The endpoint accepts only the exact bearer token and bounded worker snapshots; stale workers appear unavailable in the read-only Mini App. Keep the URL private or behind the existing operator-controlled HTTPS boundary, and rotate the token as an operator-managed secret. This control plane registers and observes workers; task execution remains on the worker and no Codex credentials are transferred. A coordinator-issued lease handoff selects a fresh, ready, capacity-available worker, sends a five-minute grant to its configured `ControlPlaneUrl`, and records issued/accepted/rejected evidence. The worker validates its identity, readiness, capabilities, and local capacity before acquiring its own local lease; the coordinator cannot interrupt or execute a worker session.

For an explicitly controlled worker update, configure `CodexTelegram:Updates` with `Enabled`, the exact local `PackagePath`, the expected `TargetVersion`, the expected `ExpectedSha256`, and an operator-owned writable `StageRoot`. Optional `RequiredCapabilities` prevents staging on an incompatible worker. The package must match the configured major/minor/build version and the worker must first be drained with zero active leases. `/worker update stage confirm` records a verified staged package and a copy of the current executable as the last-known-good rollback artifact. `/worker update rollback confirm` stages that rollback artifact. These commands do not stop the process or overwrite the protected installation; the service/fleet installer remains responsible for applying the package, starting the service, checking version/health, and either completing the update or using the staged rollback. `codex-worker-update-state.json` records bounded outcome evidence without raw paths, credentials, or transcripts.
