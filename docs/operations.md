...
6. A healthy restarted process sends the completion result back to the Telegram conversation stored in the request, confirming that the service is back online. `/update rollback confirm` uses the same handoff for an explicitly requested rollback. If the bot cannot acknowledge the start message, the updater continues after the bounded timeout and the post-restart result remains the source of truth.
...
The optional Mini App host listens on `http://127.0.0.1:5287` by default. The static page is available locally, while its live API is disabled for Telegram use until `TelegramMiniApp:Enabled` is set to `true`. It opens directly on the current conversation's selected Codex session and provides observed status, editable session name, Codex-backed goal lifecycle, owning-worker model/thinking controls, account quota usage sparklines, and a shared global-instructions editor. It has no prompt composer. During an active turn, model/thinking changes are saved for the next turn and the current turn remains labeled with its existing selection. Global instructions are saved to the operator-owned data root, automatically used by new sessions, and read at the next supported thread boundary for existing sessions; the editor never sends a chat message or interrupts work. Review, handoff, worker, and rollout surfaces remain secondary capabilities. Local sample data requires the explicit `?preview=1` query string.
Fleet rollout coordination is available from the Telegram-authenticated Mini App when workers are registered; paired browser sessions can inspect rollout state but cannot stage or activate updates. Plan a rollout with an exact version, package SHA-256, required capabilities, and explicit worker IDs, then use Advance only after the selected worker is drained and idle. The coordinator persists only rollout IDs, version/digest metadata, worker IDs, sequence, state, timestamps, and bounded outcomes. It stages one worker at a time through the local update manager or `/api/worker/v1/update/control`; `/api/worker/v1/update/status` refreshes bounded worker evidence. The external installer still owns package transfer into the protected installation, service restart, and the authenticated `/api/worker/v1/update/complete` health callback. Finalize records completion only after all selected workers report `Active`; rollback is explicit and proceeds in reverse order through `RollbackStaged` and `RollbackActive`.
...
The local worker is registered in `codex-worker-state.json` under `CodexTelegram:Workspace:DataRoot`. It contains a generated or configured stable worker ID, display name, heartbeat, drain state, and bounded task lease metadata; it does not contain prompts, transcripts, credentials, or authorization headers. `/worker status` is a read-only diagnostic. `/worker drain confirm` prevents new task leases while existing sessions continue, and `/worker resume confirm` re-enables claims. The Mini App Workers card offers the same explicitly confirmed Drain/Resume operations for the authenticated allowlisted user. For coordinator routing, configure `CodexTelegram:Worker:ControlPlaneUrl` to the worker's private HTTP(S) endpoint; the URL is advertised in bounded heartbeats and receives only token-authenticated lease grants and worker drain/resume controls. Keep that endpoint on the operator-controlled private network.
