...
For Mini App changes, keep these evidence categories separate. Review-packet checks must also prove deterministic packet IDs, Codex thread/turn provenance, bounded change/artifact counts, sanitized paths, binary/unsupported evidence labels, and the `/handoff` identity boundary. Mini App task-action checks must prove signed or paired-browser authentication, exact user/task/run/packet scope, durable idempotent acknowledgement, stale-run rejection, attention reappearance for a newer run, and handoff preparation without Telegram delivery or Codex execution. Handoff checks must additionally prove that changed-file and artifact metadata come from the same redacted packet, remain bounded, preserve turn provenance, and explicitly distinguish unavailable runtime evidence from a prepared Telegram command. Fleet rollout checks must prove owner scoping, explicit worker selection, one-worker sequencing, capability/admission/drain/zero-lease gates, authenticated remote status/control, installer-owned activation, health-failure retention, reverse-order rollback staging, and finalize refusal before all target workers report active evidence.
...
The Mini App frontend must be checked separately from the .NET release gate. Use a browser harness against the served `wwwroot` and exercise three Telegram WebApp contracts: compact (`isExpanded=false`), full-height (`isExpanded=true`), and true fullscreen (`isFullscreen=true`), including a user-initiated `requestFullscreen`/`exitFullscreen` transition. Include a narrow 320px compact viewport. Assert that the page has no horizontal overflow, that the viewport and safe-area styles are applied, that `/` and static assets render, that the current-session panel appears without a dashboard/session-list gate, that the no-active-session state is explicit, and that `activated` causes a fresh bootstrap request after the app is reopened. Record browser validation as `synthetic` unless it used an actual Telegram client; automated checks do not prove BotFather launch configuration or Telegram-live behavior.
