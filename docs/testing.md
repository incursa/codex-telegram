...
The Mini App frontend must be checked separately from the .NET release gate. Use a browser harness against the served `wwwroot` and exercise three Telegram WebApp contracts: compact (`isExpanded=false`), full-height (`isExpanded=true`), and true fullscreen (`isFullscreen=true`), including a user-initiated `requestFullscreen`/`exitFullscreen` transition. Include a narrow 320px compact viewport so worker and rollout metadata cannot reintroduce intrinsic-width overflow. Assert that the page has no horizontal overflow, that the viewport and safe-area styles are applied, that `/` and static assets render, and that `activated` causes a fresh bootstrap request after the app is reopened. Record browser validation as `synthetic` unless it used an actual Telegram client; automated checks do not prove BotFather launch configuration or Telegram-live behavior.
